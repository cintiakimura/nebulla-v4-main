...
**Law:** `nebula-project/project-execution-rules.md` — **Phase 0 – Foundation** (read orchestration sources first: `project-execution-rules.md`, then `master-plan.json`, `environment-setup.md`, `nebula-sysh-ui-sysh-studio.md`, Secrets & Integrations; Prisma/schema from **Pages and Navigation**; auth; base API structure; error handling with run/fix up to 5 attempts).
