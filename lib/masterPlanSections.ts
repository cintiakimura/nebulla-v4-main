...
- Section 2 = Research Pillars (8–12 real competitors, ranked features, evidence or "No supporting studies found for this feature.", UI patterns) + **security baseline** when auth/private data applies (auth model, tenant/RLS, roles, secrets, PII, deny-by-default) even if the user never asked.
- Section 3 = MVP features as verbs + **testable** KPIs (not slogans).
- Section 4 = every page with: name, \`/route\`, purpose, primary_actions, data_entities, authz, empty_state, error_state, nav_links — drives Mind Map + ui-brief. Vague page-name lists forbidden.
- Section 5 = **15–25 lines** visual tokens only (palette/hex, typography, density, radius, motion, components, nav). NO §4 copy. Rich UI detail → \`nebula-ui-studio/ui-brief.md\` after plan save.
- Mind Map uses Section 4 only. Primary UI path = UI Gen v2 from ui-brief + §5 tokens. V0/\`v0-prompt.md\` is optional legacy only.
`.trim();
}
