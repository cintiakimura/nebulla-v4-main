...
export function buildConcreteUiuxSection(input: {
  ...
  if (device === "mobile" && education) {
    return [
      ...
      "- **Header / identity:** top bar with screen title + optional avatar/progress chip — always visible on product pages",
      "- **Logo:** wordmark or simple mark in top bar / splash only — not a floating badge on content",
  ...
  if (device === "landing") {
    return [
      ...
      "- **Header:** logo + primary CTA; minimal nav links",
  ...
  return [
    ...
    "- **Header / identity:** app top bar with product name + primary action",
...
export function buildStitchChromeBriefSection(device: "mobile" | "web" | "landing"): string {
  const lines = [
    ...
    "1. **Identity** — top bar or hero with human title (not a route slug)",
