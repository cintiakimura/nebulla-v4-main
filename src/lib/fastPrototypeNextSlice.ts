...
export const FOUNDATION_SLICE_INSTRUCTION =
  'START_CODING — SLICE: Foundation+Primary in ONE Go. FIRST-SLICE UI FLOOR (must be visible on Live): header = one locked product name + initials (chip / §1 — never a previous chat question); nav = §4 routes as real tabs including inferred workflow pages from the lock (dossiers + forms + dashboard when keep/find documents — not login/register + stock Home cards only; no leftover Practice/Teacher unless this goal is education); globals.css = §5 palette or a dark calm default if §5 is thin / they asked dark-calm-low-stimulus (not cream-on-white); Home shows the goal verb + one primary CTA; lib/mockStore.ts only. Client-side extract (Tesseract) = we-build, do not ask for an OCR API key. Router + §4 routes. The goal verb must work with mock localStorage (no empty /api/* , no Data+API slice, no Polish). Shop: book/order updates the Home list; mechanic can mark ready. Delivery: pickup+dropoff submit appears on Home; Accept updates status. Education: Start practice writes progress on Teacher. File blocks now — then first version is on Live, mock only.';
