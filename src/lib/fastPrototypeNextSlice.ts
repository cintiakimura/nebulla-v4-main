...
export const FOUNDATION_SLICE_INSTRUCTION =
  'START_CODING — SLICE: Foundation+Primary in ONE Go. FIRST-SLICE APPLY = the locked loop from Master Plan §4 (core intake → extract/review → save where it lives → next status). If §4 is Dashboard / Dossiers / Forms, emit app/page.tsx + app/dossiers/page.tsx + app/forms/page.tsx + app/dashboard/page.tsx — never only /login /register / or leftover /A. Infer history/per-client dossier when the north star is keep/find work later; do not quiz “do you want a history page?” Auth pages only if the lock requires signed-in roles. FIRST-SLICE UI FLOOR (must be visible on Live): header = one locked product name + initials (chip / §1 — never a previous chat question or concatenated leftovers); nav = those §4 routes as real tabs or left sidebar (web) / hamburger (mobile); no leftover Practice/Teacher unless this goal is education; globals.css = §5 palette or a dark calm default if §5 is thin / they asked dark-calm-low-stimulus (not cream-on-white); Home shows the goal verb + one primary CTA; lib/mockStore.ts shaped like the real objects (dossier, document, form — not Maya Chen cards). Client-side extract (Tesseract) = we-build, do not ask for an OCR API key. The goal verb must work with mock localStorage (no empty /api/* , no Data+API slice, no Polish). Shop: book/order updates the Home list; mechanic can mark ready. Delivery: pickup+dropoff submit appears on Home; Accept updates status. Education: Start practice writes progress on Teacher. File blocks now — then first version is on Live, mock only.';
...
export const NARROW_FOUNDATION_SLICE_INSTRUCTION =
  'START_CODING — SLICE: Foundation — smallest runnable shell only (4–8 files): package.json, app/layout.tsx, app/globals.css, app/page.tsx, plus the core §4 loop routes (dossiers/forms/dashboard when locked — not /login /register /A). File blocks now.';
