...
Policies / step definitions: `recovery-lab/proposed/policies-and-steps.md` (Phase B).  
This file = **order only**. Depth lives in rooms named per step.

---

## Doctrine (max 8 bullets)
...
- Prefer infer + label (`assumption:` / `inferred:`) over interview. Full gap-fill: Phase B §2.
...
## Delta vs live project-workflow.md

- Replaces thin pointer table with **executable Steps 1–14** (verbs, artifacts, IF/ELSE).
- Intake = gap-fill / inference default; live still says “Discovery one question at a time.”
- Security: mid-run MVP + **end-of-run offer** (Step 13); live implies security baseline auto-inject as completeness.
- Research: explicit **direct → analogues → baseline**; live only points at Research Pillars in execution-rules.
- UI: **UI Gen v2** primary kept; auto-V0 remains non-spine (live already optional — proposed makes order non-negotiable).
- Coding: Foundation → Validate → **Primary** before “polish / Run and Test.”
- Conductor claim: **this file** is north star; execution-rules / inference-first / UI v2 are rooms — live circular “authority = execution-rules.”

## Delta vs project-workflow-before.md

- **Force kept:** Immediate Mind Map from §4 only; must not wait for UI; numbered post-plan order; §5 ≤15–25 lines; no large code dumps in chat; Foundation after UI path triggered.
- **V0 spine dropped:** Immediate `v0-prompt.md` + auto V0 API + `V0_API_KEY` required → remapped to ui-brief + UI Gen v2; V0 optional legacy only.
- **Interview default dropped:** Before Master Plan interview → Steps 1–4 gap-fill.
- **Security timing changed:** Before collected in interview; now Deferred + Step 13 offer.

## Delta (Phase B alignment)

- **No merges/splits:** Steps 1–14 match Phase B catalog 1:1.

---

## Non-goals

- Not live authority (do not replace `nebula-project/project-workflow.md` until Phase E+)
- Not full `project-execution-rules` replacement
- Not code, gates, or prompt wiring
- Not auto-V0 restoration

---

## Completion checklist

- [x] Only `recovery-lab/proposed/project-workflow.proposed.md` (+ optional link note)
- [x] Zero `nebula-project` / `nebulla-project` edits
- [x] Zero code changes
- [x] Steps align with Phase B `policies-and-steps.md`
